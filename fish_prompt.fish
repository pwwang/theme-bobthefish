...
#		 set -g theme_newline_leading yes
...
function __bobthefish_prompt_status -S -a last_status -d 'Display flags for a non-zero exit status, root user, and background jobs'
	...
	set -l segm
	set -l showexit
	set -l duration (__bobthefish_cmd_duration)
	...
	if [ "$theme_show_exit_status" = 'yes' ]
		[ "$duration" -o $last_status -ne 0 ]
			and set showexit 1
	end

	if [ $last_status -ne 0 ]
		set segm (__bobthefish_start_segment $color_initial_segment_exit1 | string trim)
	else
		set segm (__bobthefish_start_segment $color_initial_segment_exit0 | string trim)
	end

	if [ "$showexit" -o "$superuser" -o "$bg_jobs" ]
		set __bobthefish_exit_shown 1
		echo -ns $segm

		if [ "$showexit" ]
			set_color normal
			echo -ns $segm
			if [ $last_status -ne 0 -a "$duration" ]
				echo -ns ' ' $last_status ' (' $duration ') '
			else if [ $last_status -ne 0 ]
				echo -ns ' ' $last_status ' '
			else
				echo -ns ' ' $duration ' '
			end
		...
		if [ "$bg_jobs" ]
			...
			set_color $color_initial_segment_jobs[2..-1]
			echo -ns ' ' $bg_job_glyph
			#set __bobthefish_current_bg $color_initial_segment_jobs[1]
		end
	end
end
...
function __bobthefish_prompt_user -S -d 'Display current user and hostname'
	...
	if set -q display_user
		if [ "$__bobthefish_exit_shown" ]
			__bobthefish_start_segment $color_username
		else
			set_color -b $color_username[1] $color_username[2..-1]
			echo -n ' '
			set __bobthefish_current_bg $color_username[1]
		end
		echo -ns (whoami)
	end
	...
	if set -q display_hostname
		if set -q display_user
			...
			set __bobthefish_current_bg $color_hostname[1]
		else
			if [ "$__bobthefish_exit_shown" ]
...
function __bobthefish_prompt_date -S -d 'Display system date and time'
	[ "$theme_display_date" = 'no' ]; and return
	set_color normal
	__bobthefish_start_segment $color_date
...
function fish_prompt -d 'bobthefish, a fish theme optimized for awesome'
	...
	# User / hostname info
	__bobthefish_prompt_user $last_status
	__bobthefish_prompt_date
